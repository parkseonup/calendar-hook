...
export const getMonthEndDate = (date: Date): number => {
  if (date instanceof Date) {
...
/** 특정 달의 1일 날짜를 구하는 함수 */
export const setFirstDate = (date: Date): Date => {
  date.setDate(1);
  return date;
};

/** 한 주의 시작 요일을 기준으로 특정 달이 시작하는 날짜의 index를 구하는 함수  */
export const getMonthlyStartIndex = (date: Date, weekStartDay: number = 0): number => {
...
/** 특정 달의 주 수를 구하는 함수 */
export const getNumberOfWeeks = (date: Date, weekStartDay: number = 0): number => {
...
export const getWeekDays = (weekStartDay: number): WeekDays => {
  const newWeekDays: { key: number; value: string }[] = [];
...
export const getWeeklyDate = (startDate: Date, currentMonth: number): WeeklyDate =>
...
export const getMonthlyDate = (date: Date, weekStart: number, showFixedNumberOfWeeks?: number): MonthlyDate => {
